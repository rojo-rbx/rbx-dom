...
fn variant_type_from_str(value: &str) -> Option<VariantType> {
    Some(match value {
        ...
        // While DateTime is possible to Serialize, the only use it has as a DataType
        // is for the TextChatMessage class, which cannot be serialized (at least not saved to file as it is locked to nil parent)
