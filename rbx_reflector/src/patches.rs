                ...
                match prop_change.has_default {
                    Some(false) => {
                        ...
                        for descendant in subclass_list {
                            ...
                            class.default_properties.remove(prop_name.as_str());
                        }
                    }
                    Some(true) => {
                        log::warn!("{class_name}.{prop_name} is patched to be `HasDefault: true` but this does nothing")
                    }
                    None => {}
                }
            }
