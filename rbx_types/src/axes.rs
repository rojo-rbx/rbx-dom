...
mod mlua_impl {
    ...
    impl Axes {
        fn into_lua(lua: &Lua) -> LuaResult<LuaTable> {
            use crate::EnumItem;

            let new = |_: &Lua, args: Variadic<EnumItem>| -> LuaResult<Axes> {
                let mut flags: AxisFlags = AxisFlags::empty();

                for arg in args {
                    match &arg.ty[..] {
                        "Axis" => {
                            match arg.value {
                                /* X */ 0_u32 => flags |= AxisFlags::X,
                                /* Y */ 1_u32 => flags |= AxisFlags::Y,
                                /* Z */ 2_u32 => flags |= AxisFlags::Z,
                                _ => (),
                            }
                        }
                        "NormalId" => {
                            match arg.value {
                                /* Right */ 0_u32 => flags |= AxisFlags::X,
                                /* Top */ 1_u32 => flags |= AxisFlags::Y,
                                /* Back */ 2_u32 => flags |= AxisFlags::Z,
                                /* Left */ 3_u32 => flags |= AxisFlags::X,
                                /* Bottom */ 4_u32 => flags |= AxisFlags::Y,
                                /* Front */ 5_u32 => flags |= AxisFlags::Z,
                                _ => (),
                            }
                        }
                        _ => (),
            ...
            let mut axes = lua.create_table()?;
            // axes.set("new", lua.create_function(new)?)?;
            axes.set_readonly(true);
            Ok(axes)
        }
    }
}
