...
| Rotation matrix | `Optional<Matrix3>` | The rotational component of the `CFrame`, stored as a sequence of nine `f32`s. If Rotation ID is `00`, this field is the rotation matrix in **row-major order**, the XVector, the YVector, and ZVector, in that order; otherwise, it is absent.        |
