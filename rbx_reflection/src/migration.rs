...
pub fn perform_migration(
    migration: PropertyMigration,
    input: &Variant,
) -> Result<Variant, MigrationError> {
    match migration {
        PropertyMigration::IgnoreGuiInsetToScreenInsets => {
            if let Variant::Bool(value) = input {
                ...
                Err(MigrationError::InvalidTypeForMigration {
                    migration: PropertyMigration::IgnoreGuiInsetToScreenInsets,
                    expected: "Enum",
                    actual: input.clone(),
                })
            }
        }
        PropertyMigration::FontToFontFace => {
                ...
                Ok(match value {
                    ...
                    _ => {
                        return Err(MigrationError::InvalidValueForMigration {
                            migration: PropertyMigration::FontToFontFace,
